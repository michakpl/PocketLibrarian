...
import {addBook, addBookFromISBN, updateBook} from '@/lib/api/books'
...
function extractISBNFields(formData: FormData) {
    return {
...
export async function addBookFromISBNAction(
    _prev: BookFormState,
    formData: FormData
): Promise<BookFormState> {
    const session = await getSession()
    ...
    try {
        await addBookFromISBN(session.accessToken, extractISBNFields(formData))
    } catch (e) {
        return {error: e instanceof Error ? e.message : 'Failed to add book'}
    }
