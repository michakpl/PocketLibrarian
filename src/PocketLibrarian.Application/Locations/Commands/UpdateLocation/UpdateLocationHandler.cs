...
{
    public async ValueTask<LocationDto> Handle(UpdateLocationCommand cmd, CancellationToken cancellationToken)
    {
        var location = await db.Locations.FirstOrDefaultAsync(l => l.Id == cmd.Id && l.OwnerId == cmd.OwnerId, cancellationToken);
        ...
        {
            var parentExists =
                await db.Locations.AnyAsync(l => l.Id == cmd.ParentId.Value && l.OwnerId == cmd.OwnerId, cancellationToken);
